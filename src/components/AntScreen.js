  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((user) => {
      if (user) {
        ...
        const beslenmeRef = db.ref("Antrenor/ "+userUID+"/BeslenmeProgrami/");
        ...
        const antrenmanRef = db.ref("Antrenor/ "+userUID+"/AntrenmanProgrami/");
        ...
        // Kullanıcı verileri
        const userRef = db.ref("Antrenor/ "+userUID);
        userRef.once('value')
          .then((snapshot) => {
            if (snapshot.exists()) {
  ...
  const handleUpdate = () => {
    ...
    userRef.update(updatedUserData)
      .then(() => {
        ...
        const beslenmeRef = userRef.child("BeslenmeProgrami");
        beslenmeRef.set(beslenmeList)
          .then(() => {
            console.log("Beslenme programı güncellendi.");
  
            // Update training program
            const antrenmanRef = userRef.child("AntrenmanProgrami");
            antrenmanRef.set(antrenmanList)
              .then(() => {
                console.log("Antrenman programı güncellendi.");
              })
              .catch((error) => {
                console.error("Antrenman programı güncelleme hatası:", error);
              });
          })
          .catch((error) => {
            console.error("Beslenme programı güncelleme hatası:", error);
          });
      })
      .catch((error) => {
    ...
    <div>
      ...
      {userData ? (
        <div>
          ...
          {/* Antrenman Programı Güncelleme */}
          <h3>Antrenman Programını Güncelle</h3>
          {antrenmanList.map((antrenman, index) => (
            <div key={index}>
              <span>{index + 1}. Gün: </span>
              <input
                type="text"
                name="AntrenmanProgrami"
                value={antrenman}
                onChange={(e) => {
                  const updatedAntrenmanList = [...antrenmanList];
                  updatedAntrenmanList[index] = e.target.value;
                  setAntrenmanList(updatedAntrenmanList);
                }}
              />
            </div>
          ))}
          ...
          <h3>Beslenme Programını Güncelle</h3>
          {beslenmeList.map((yemek, index) => (
            <div key={index}>
              <span>{index + 1}. Gün: </span>
              <input
                ...
                value={yemek}
                onChange={(e) => {
                  const updatedBeslenmeList = [...beslenmeList];
                  updatedBeslenmeList[index] = e.target.value;
                  setBeslenmeList(updatedBeslenmeList);
                }}
              />
            </div>
          ))}
